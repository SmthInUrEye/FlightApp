...
void testCreateFlight() throws Exception{
    ...
    mockMvc.perform(post("/api/flights").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(requestDTO))).andExpect(status().isOk()).andExpect(jsonPath("$.id",is(1)));
}
...
void testGetAllFlights() throws Exception{
    ...
    List<FlightDTO> dtos=flights.stream().map(flight->toDTO(flight,idGen.getAndIncrement())).collect(Collectors.toList());

    when(flightService.getAllFlights()).thenReturn(dtos);

    mockMvc.perform(get("/api/flights")).andExpect(status().isOk()).andExpect(jsonPath("$",hasSize(dtos.size()))).andExpect(jsonPath("$[0].id").value(1)).andExpect(jsonPath("$[1].id").value(2));
}
...
void testGetFlightById_found() throws Exception{
    ...
    mockMvc.perform(get("/api/flights/10")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(10));
}
...
void testUpdateFlight_success() throws Exception{
    ...
    when(flightService.updateFlight(eq(5L),any(FlightDTO.class))).thenReturn(responseDTO);

    FlightDTO requestDTO=flightMapper.toDTO(flight);
    requestDTO.setId(5L);

    mockMvc.perform(put("/api/flights/5").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(requestDTO))).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(5));
}
...
void testUpdateFlight_notFound() throws Exception{
    ...
    when(flightService.updateFlight(eq(999L),any(FlightDTO.class))).thenThrow(new RuntimeException("Flight not found with ID 999"));

    mockMvc.perform(put("/api/flights/999").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(requestDTO))).andExpect(status().isNotFound());
}

@Test
void testDeleteFlight() throws Exception{
    mockMvc.perform(delete("/api/flights/3")).andExpect(status().isNoContent());
}
...
void testFilterByName() throws Exception{
    ...
    mockMvc.perform(get("/api/flights/filter/{name}",filterName)).andExpect(status().isOk()).andExpect(jsonPath("$",hasSize(1))).andExpect(jsonPath("$[0].id").value(33));
}

@Test
void testGenerateDemoFlights() throws Exception{
    mockMvc.perform(post("/api/flights/generate-demo")).andExpect(status().isOk());
}
...
void testFlightDTOStructure() throws Exception{
    ...
    mockMvc.perform(get("/api/flights/1")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.segments",hasSize(1)));
}
}